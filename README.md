...
The script runs all 4 tasks sequentially and prints progress to stdout:

1. **Task 1**: Trains 30 model configurations (3 batch sizes × 5 learning rates × 2 regularization settings) for 200 epochs. Saves 2 training curve plots.
...
| File | Description |
|------|-------------|
| `task1/training_curves_no_reg.png` | Training CE vs epoch, no regularization |
| `task1/training_curves_l2.png` | Training CE vs epoch, L2 (λ=1e-3) |
| `task2/cv_results_table.png` | Heatmap of mean validation CE over CV grid |
| `task3/lambda_sweep_ce.png` | Train vs val CE as function of λ |
| `task3/lambda_sweep_acc.png` | Train vs val accuracy as function of λ |
...
- **L2 convention**: `λ‖w‖₂²` with gradient term `2λw` (bias excluded)
...
**`ModuleNotFoundError`**
Make sure your virtual environment is activated and `uv sync` / `pip install -r requirements.txt` has been run.

**`uv: command not found`**
Install uv first: `pip install uv`

**Convergence warnings from sklearn**
The `max_iter=10000` in Task 4 is sufficient for most C values. If warnings appear, they can be ignored safely — results are still valid.
